...
async function updateProductDiscounts() {
  try {
    ...
    for (const food of Foods) {
      // Check if any offer includes the product
      const hasFoodInOffer = await Offer.exists({
        food_ids: food._id,
      });
      if (!hasFoodInOffer) {
        // If the product is not in any offer, set its discount_price to zero
        food.discount_type = "";
...
const offerController = {
      ...
      try {
        if (req.file) {
          ...
          if (foods.length !== food_ids.length) {
            return res
            .status(404)
            .json({ message: "One or more food items not found" });
          }
          const food_discount_ids = foods.map((food) => food._id);
          
          // Create the offer with the offer image
          ...
          // Apply the discount prices to the food items
          for (const food of foods) {
            // Update food discount_type and discount_value with offer details
  ...
  updateOffer: async (req, res) => {
    try {
      ...
      const { type, value, start_date, end_date, offer_name, isPopular } =
        req.body;
      let food_ids = req.body.food_ids ? JSON.parse(req.body.food_ids) : [];
      const foods = await Food.find({ _id: { $in: food_ids } });
  ...
  deleteOffer: async (req, res) => {
    try {
      ...
      // Reset the discount price of associated food items to zero
